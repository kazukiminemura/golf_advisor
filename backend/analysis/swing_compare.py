...
KEYPOINT_WEIGHTS: Dict[int, float] = {
    ...
    10: 0.8, 11: 0.6, 13: 0.8, 14: 0.6,  # legs
}
...
def compare_swings(ref_kp: Sequence[Frame], test_kp: Sequence[Frame]) -> Tuple[float, Dict[str, float]]:
    ...
    for name, (s, e) in phases.indices.items():
        phase_scores[name] = _phase_score(ref_kp, test_kp, s, e, strictness=STRICTNESS_FACTOR)
...
class GolfSwingAnalyzer:
    ...
    def _perform_analysis(self) -> Dict[str, object]:
        ...
        swing_phases = {k: phase_scores.get(k, 0.0) for k in ("address", "backswing", "downswing", "follow_through")}
